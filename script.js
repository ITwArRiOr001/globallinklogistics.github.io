...
// Modal for quick quotes (Products page enhanced)
function openModal(modalId, productName = "", productType = "export", priceRange = "") {
    const modal = document.getElementById(modalId);
    if (!modal) return;
    
    // Fill quote modal fields only if present (Products page)
    const nameEl = document.getElementById("product-name");
    const typeEl = document.getElementById("product-type");
    const priceEl = document.getElementById("price-range");
    
    if (nameEl && productName) nameEl.value = productName;
    if (typeEl && productType) typeEl.value = productType;
    if (priceEl) priceEl.textContent = priceRange ? `Estimated Price: ${priceRange}` : "";
